...
try {
    $sql = "SELECT * FROM camiones ORDER BY placa ASC";
    $stmt = $pdo->query($sql);
    $camiones = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $error_carga = true;
    $mensaje = 'No fue posible cargar la flota de camiones.';
    $tipo_mensaje = 'danger';
}
...
$hoy = date('Y-m-d');
$limiteProximo = date('Y-m-d', strtotime('+30 days'));
...
foreach ($camiones as $row) {
    ...
    if (($row['condicion'] ?? '') === 'Bueno') {
        $condicion_buena++;
    }

    if (!empty($row['prox_mantenimiento'])) {
        if ($row['prox_mantenimiento'] >= $hoy && $row['prox_mantenimiento'] <= $limiteProximo) {
...
function claseCondicion($condicion)
{
    switch ($condicion) {
        case 'Bueno':
            return 'badge badge-bueno';
        case 'Regular':
            return 'badge badge-regular';
        case 'Malo':
            return 'badge badge-malo';
        default:
            return 'badge badge-neutral';
    }
}

function textoMantenimiento($fecha)
{
    if (empty($fecha)) {
...
function mantenimientoEsProximo($fecha)
{
...
<head>
    ...
    <title>Holcim · Flota de Camiones</title>
...
<div class="layout">
    <aside class="sidebar" id="sidebar">
        ...
        <div class="menu-group">
            ...
            <a class="nav-link active" href="../camiones/listar.php"><i class="fas fa-truck-moving"></i> Camiones</a>
    ...
    <main class="content">
        <section class="mobile-hero">
            ...
            <p>Consulta y administra el estado operativo de las unidades registradas, su condición general y fechas de mantenimiento.</p>
            <div class="mobile-hero-meta">
                ...
                <div class="mobile-hero-chip">
                    ...
                    <div class="value"><?php echo $total_camiones; ?></div>
                </div>
            </div>
        </section>
        ...
        <div class="topbar">
            <div class="welcome-box">
                ...
                <p>Supervisa disponibilidad, ocupación, condición y próximos mantenimientos de las unidades registradas en el sistema.</p>
            </div>
        ...
        <section class="hero-strip">
            <div class="hero-card main">
                ...
                <div class="hero-value"><?php echo $total_camiones; ?></div>
                <div class="hero-sub">Unidades registradas en la flota general del sistema.</div>
            </div>

            <div class="hero-card">
                <div class="hero-title">Disponibles</div>
                <div class="hero-value"><?php echo $disponibles; ?></div>
                <div class="hero-sub">Camiones listos para operación o asignación logística.</div>
            </div>

            <div class="hero-card">
                <div class="hero-title">En mantenimiento</div>
                <div class="hero-value"><?php echo $mantenimiento; ?></div>
                <div class="hero-sub">Unidades fuera de servicio por revisión o reparación.</div>
            </div>

            <div class="hero-card">
                <div class="hero-title">Condición buena</div>
                <div class="hero-value"><?php echo porcentajeSeguro($condicion_buena, max(1, $total_camiones)); ?>%</div>
                <div class="hero-sub">Porcentaje de unidades marcadas con condición “Bueno”.</div>
            </div>
        </section>
        ...
        <section class="kpi-grid">
            <div class="kpi-card">
                <div class="kpi-top">
                    ...
                    <div class="kpi-badge">Flota</div>
                </div>
                <div class="kpi-label">Total de unidades</div>
                <div class="kpi-value"><?php echo $total_camiones; ?></div>
                <div class="kpi-foot">Base de camiones registrados para operación y control.</div>
            </div>

            <div class="kpi-card">
                <div class="kpi-top">
                    <div class="kpi-icon"><i class="fas fa-check-circle"></i></div>
                    <div class="kpi-badge">Disponibilidad</div>
                </div>
                <div class="kpi-label">Disponibles</div>
                <div class="kpi-value"><?php echo $disponibles; ?></div>
                <div class="kpi-foot">Unidades listas para programación de despacho.</div>
            </div>
            ...
            <div class="kpi-card">
                ...
                <div class="kpi-value"><?php echo $ocupados; ?></div>
                <div class="kpi-foot">Camiones actualmente comprometidos en operación.</div>
            </div>

            <div class="kpi-card">
                <div class="kpi-top">
                    <div class="kpi-icon"><i class="fas fa-screwdriver-wrench"></i></div>
                    <div class="kpi-badge">Control</div>
                </div>
                <div class="kpi-label">Próx. mantenimiento</div>
                <div class="kpi-value"><?php echo $proximo_mantenimiento; ?></div>
                <div class="kpi-foot">Unidades con mantenimiento programado en los próximos 30 días.</div>
            </div>
        </section>
        ...
        <div class="toolbar">
            <div class="search-group">
                ...
                <input
                    ...
                    id="searchInput"
                    class="search-input"
                    placeholder="Buscar por placa, modelo, marca, estado o condición..."
                >
            </div>
        ...
        <section class="section">
            <div class="section-header">
                <div>
                    ...
                    <p>Consulta el catálogo completo de unidades con su estado, condición y mantenimiento programado.</p>
                </div>
            </div>
            ...
            <div class="panel">
                <?php if ($total_camiones > 0): ?>
                    <div class="empty-search" id="emptySearchState">
                        ...
                        <p>No se encontraron unidades con ese criterio de búsqueda.</p>
                    </div>
                                ...
                                <tbody id="desktopTableBody">
                                    <?php foreach ($camiones as $row): ?>
                                        <?php
                                            ...
                                            $prox_mantenimiento = $row['prox_mantenimiento'] ?? '';
                                            $searchText = strtolower(trim($placa . ' ' . $modelo . ' ' . $marca . ' ' . $estado . ' ' . $condicion));
                                        ?>
                                        <tr class="search-item" data-search="<?php echo htmlspecialchars($searchText); ?>">
                                            ...
                                            <td class="placa-cell">
                                                <span class="info-flex">
                                                    ...
                                                    <?php echo htmlspecialchars($placa); ?>
                                                </span>
                                            </td>
                                            <td>
                                                <?php echo !empty($modelo) ? htmlspecialchars($modelo) : '<span class="muted">Sin modelo</span>'; ?>
                                                <?php if (!empty($marca)): ?>
                                                    <span class="subtext"><?php echo htmlspecialchars($marca); ?></span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <strong><?php echo number_format((float) $capacidad, 2); ?> m³</strong>
                                            </td>
                                            <td>
                                                <span class="<?php echo claseEstado($estado); ?>">
                                                    <i class="fas fa-circle"></i>
                                                    <?php echo htmlspecialchars($estado ?: 'Sin estado'); ?>
                                                </span>
                                            </td>
                                            <td>
                                                <span class="<?php echo claseCondicion($condicion); ?>">
                                                    <?php echo htmlspecialchars($condicion ?: 'Sin condición'); ?>
                                                </span>
                                            </td>
                                            <td class="<?php echo mantenimientoEsProximo($prox_mantenimiento) ? 'mantenimiento-proximo' : ''; ?>">
                                                <?php if (!empty($prox_mantenimiento)): ?>
                                                    <span class="info-flex">
                                                        <i class="fas fa-calendar-alt"></i>
                                                        <?php echo htmlspecialchars(textoMantenimiento($prox_mantenimiento)); ?>
                                                    </span>
                                                <?php else: ?>
                                                    <span class="muted">No programado</span>
                                                <?php endif; ?>
                                            </td>
                                            <td class="actions-cell">
                                                <a href="editar.php?id=<?php echo (int) $row['id']; ?>" class="btn-action btn-edit">
                                                    <i class="fas fa-pen"></i> Editar
                                                </a>
                                                <button
                                                    type="button"
                                                    class="btn-action btn-delete"
                                                    onclick="confirmarEliminar(<?php echo (int) $row['id']; ?>, '<?php echo htmlspecialchars(addslashes($placa)); ?>')"
                                                >
                            ...
                            <div class="mobile-card search-item" data-search="<?php echo htmlspecialchars($searchText); ?>">
                                <div class="mobile-card-top">
                                    <div class="mobile-card-title">
                                        <span class="id-badge">#<?php echo (int) $row['id']; ?></span>
                                        <div class="mobile-name"><?php echo htmlspecialchars($placa); ?></div>
                                    </div>
                                    <span class="<?php echo claseEstado($estado); ?>">
                                        <i class="fas fa-circle"></i>
                                        <?php echo htmlspecialchars($estado ?: 'Sin estado'); ?>
                                ...
                                <div class="mobile-grid">
                                    <div class="mobile-item">
                                        <div class="mobile-label">Modelo / Marca</div>
                                        <div class="mobile-value">
                                            <?php echo !empty($modelo) ? htmlspecialchars($modelo) : 'Sin modelo'; ?>
                                            <?php if (!empty($marca)): ?>
                                                <br><span class="muted"><?php echo htmlspecialchars($marca); ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>

                                    <div class="mobile-item">
                                        <div class="mobile-label">Capacidad</div>
                                        <div class="mobile-value"><?php echo number_format((float) $capacidad, 2); ?> m³</div>
                                    </div>

                                    <div class="mobile-item">
                                        <div class="mobile-label">Condición</div>
                                        <div class="mobile-value">
                                            <span class="<?php echo claseCondicion($condicion); ?>">
                                                <?php echo htmlspecialchars($condicion ?: 'Sin condición'); ?>
                                            </span>
                                    ...
                                    <div class="mobile-item">
                                        <div class="mobile-label">Próximo mantenimiento</div>
                                        <div class="mobile-value <?php echo mantenimientoEsProximo($prox_mantenimiento) ? 'mantenimiento-proximo' : ''; ?>">
                                            <?php echo htmlspecialchars(textoMantenimiento($prox_mantenimiento)); ?>
                                        </div>
                                    </div>
                                </div>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Comienza agregando la primera unidad al módulo de flota.</p>
        ...
        <div class="footer">
            © 2026 Holcim México · Sistema de Gestión de Cemento · Módulo de Camiones v3.0
        </div>
    </main>
</div>
...
<nav class="mobile-bottom-nav">
    ...
    <a href="../camiones/listar.php" class="active">
        <i class="fas fa-truck-moving"></i>
        <span>Camiones</span>
    </a>
    <a href="../logout.php">
...
<div class="confirm-modal" id="confirmModal">
    <div class="confirm-box">
        ...
        <h3>Eliminar camión</h3>
        <p id="confirmMessage">¿Estás seguro de eliminar este camión?</p>
...
<script>
    ...
    function confirmarEliminar(id, placa) {
        ...
        if (confirmMessage) {
            confirmMessage.textContent = '¿Estás seguro de eliminar el camión con placa "' + placa + '"? Esta acción no se puede deshacer.';
        }
    ...
    window.addEventListener('load', () => {
        ...
        <?php else: ?>
        showToast(
            'Módulo de camiones listo',
            'Puedes buscar, editar o eliminar unidades desde esta vista.',
            'info',
            3200
        );
        <?php endif; ?>
